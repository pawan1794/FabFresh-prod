                ...
                for j in typer:
                    if j.type_id is clothResult[i]['type']:
                        if int(order[0].order_type) is 0:
                            ...
                            c = ClothSplitPrice(orders=order[0],typeName=j.type_name,typeQuantity=clothResult[i]['c'],typePrice=j.type_price_wash,total=j.type_price_wash * clothResult[i]['c'],amount=order[0].amount)
                            print order[0].amount
                        else:
                            count = count + j.type_price_iron * clothResult[i]['c']
                            c = ClothSplitPrice(orders=order[0],typeName=j.type_name,typeQuantity=clothResult[i]['c'],typePrice=j.type_price_iron,total=j.type_price_iron * clothResult[i]['c'],amount=order[0].amount)
    ...
    def get_queryset(self):
        ...
        for i in c:
            ...
            if orderInstance.afterDiscount is None:
                i.afterDiscount = orderInstance.amount
            else:
                i.afterDiscount = orderInstance.afterDiscount
            i.amount = orderInstance.amount
            i.save()
